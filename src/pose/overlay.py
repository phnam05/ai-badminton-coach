...
def annotate_frame(bgr, frame_pose, seq: PoseSequence, events=None) -> None:
    """Draw the skeleton and event banners onto one frame, in place.

    Shared by the overlay video renderer and the interactive review viewer so
    both always show identical annotations.
    """
...
def render_overlay(video_path: str | Path, seq: PoseSequence, out_path: str | Path, events=None) -> Path:
    ...
    for frame_pose in seq.frames:
        ...
        if not ok:
            break
        annotate_frame(bgr, frame_pose, seq, events)
        writer.write(bgr)
